...
class InterestedItemsScreen extends StatefulWidget {
  const InterestedItemsScreen({super.key});
...
class _InterestedItemsScreenState extends State<InterestedItemsScreen> {
  ...
  void _loadInterestedItems() {
    ...
    if (_currentUser != null && _currentUser!.interestedCategories.isNotEmpty) {
      // Filter items based on user's interested categories
      _filteredItems = allItems.where((item) {
        final categoryName = item.categoryName.toLowerCase();
        return _currentUser!.interestedCategories.any((interest) =>
            interest.toLowerCase() == categoryName);
      }).toList();

      // Sort by posted date (newest first)
      _filteredItems.sort((a, b) => b.postedAt.compareTo(a.postedAt));
    } else {
      _filteredItems = [];
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Items You\'re Interested In'),
      ...
      body: _isLoading
          ...
          : _currentUser == null
              ...
              : _currentUser!.interestedCategories.isEmpty
                  ? Center(
                      child: Padding(
                        padding: const EdgeInsets.all(32),
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 8),
                            Text(
                              'Update your profile to set category preferences',
                              style: TextStyle(
                                fontSize: 14,
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                const SizedBox(height: 8),
                                Text(
                                  'Check back later for new items in your interested categories',
                                  style: TextStyle(
                                    fontSize: 14,
                                    color: Colors.grey.shade500,
                                  ),
